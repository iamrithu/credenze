...
class ServiceCustomAddEmployee extends ConsumerStatefulWidget {
  final String token;
  final int id;
  final String date;
  ...
  const ServiceCustomAddEmployee({
    Key? key,
  ...
  @override
  _ServiceCustomAddEmployeeState createState() =>
      _ServiceCustomAddEmployeeState();
}

class _ServiceCustomAddEmployeeState
    extends ConsumerState<ServiceCustomAddEmployee> {
  ...
  void initState() {
    Api()
        .serviceWorkParticipants(widget.token, widget.id, widget.date)
        .then((value) {
      setState(() {
        list = value;
      });
